-- Add Slack webhook URL column to checks table
ALTER TABLE checks 
ADD COLUMN slack_webhook_url TEXT DEFAULT NULL;

-- Add a check constraint to validate Slack webhook URL format
ALTER TABLE checks
ADD CONSTRAINT valid_slack_webhook CHECK (
  slack_webhook_url IS NULL OR 
  slack_webhook_url ~* '^https://hooks\.slack\.com/services/[A-Z0-9]+/[A-Z0-9]+/[A-Za-z0-9]+$'
);

-- Update the update_overdue_checks function to include Slack notifications
...
BEGIN
  ...
  FOR check_record IN 
    SELECT c.id, c.name, c.user_id, c.last_pinged_at, c.interval_minutes, c.grace_period_minutes, c.slack_webhook_url
    FROM checks c
    WHERE c.status = 'up' 
  ...
  LOOP
    ...
    IF check_record.slack_webhook_url IS NOT NULL THEN
      BEGIN
        SELECT * INTO http_response
        FROM http_post(
          'https://mrtovhqequmhdgccwffs.supabase.co/functions/v1/send-slack-notification',
          json_build_object(
            'check_id', check_record.id::text,
            'check_name', check_record.name,
        ...
        RAISE NOTICE 'Sent Slack notification for check % (%) - Status: %', 
          check_record.name, check_record.id, http_response.status;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Failed to send Slack notification for check %: %', 
          check_record.name, SQLERRM;
      END;
    END IF;
